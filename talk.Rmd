...
```{r engine='bash', comment='', eval = FALSE, echo = TRUE}
docker run -p 8000:8000 diamondpredict 
```

`-p 8000:8000` maps port 8000 in "dockerland" to port 8000 on the actual machine
...
`docker-compose` sets up nginx container and internal network
